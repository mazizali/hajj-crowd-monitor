    ...
    for url in candidates:
        try:
            ...
            with urllib.request.urlopen(req, timeout=8) as resp:
                data = resp.read()
            # Skip YouTube grey placeholder images (they're < ~5 KB)
            if len(data) > 8_000:
                return data
        except Exception:
            continue
...
class handler(BaseHTTPRequestHandler):
    def do_GET(self):
        ...
        if not video_id:
            body = json.dumps({'error': f'unknown stream: {stream_id}'}).encode()
        ...
        frame_b64 = base64.b64encode(raw).decode() if raw else None
    ...
    def log_message(self, format, *args):  # silence default stdout logging
        pass
